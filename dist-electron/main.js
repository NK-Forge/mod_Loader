...
const path__namespace$1 = /* @__PURE__ */ _interopNamespaceDefault(path$4);
electron.ipcMain.handle("paths:reveal", async (_ev, target) => {
  if (!target) return;
...
const APP_DIR$1 = path$3.join(electron.app.getPath("userData"));
const CONFIG_PATH$1 = path$3.join(APP_DIR$1, "config.json");
async function ensureDir$2(p) {
  if (!p) return;
  await fs$4.promises.mkdir(p, { recursive: true }).catch(() => void 0);
}
async function readJSON$1(file, fallback) {
  try {
    const raw = await fs$4.promises.readFile(file, "utf-8");
...
async function writeJSON$1(file, data) {
  ...
  await fs$4.promises.mkdir(dir, { recursive: true });
  await fs$4.promises.writeFile(file, JSON.stringify(data, null, 2), "utf-8");
}
safeHandle("config:get", async () => {
  return getConfig();
});
safeHandle("config:set", async (_evt, next) => {
  await writeJSON$1(CONFIG_PATH$1, next);
  replaceConfig(next);
  return true;
});
safeHandle("config:completeSetup", async (_evt, patch) => {
  const cur = await readJSON$1(CONFIG_PATH$1, {});
  const next = {
    ...cur,
    ...patch,
    setupComplete: true,
  ...
  await Promise.all([
    ensureDir$2(next.activeModsPath),
    ensureDir$2(next.modsVaultPath),
    ensureDir$2(next.modPlayVaultPath),
    ensureDir$2(next.saveDataPath)
  ]);
  await writeJSON$1(CONFIG_PATH$1, next);
  replaceConfig(next);
  return next;
...
async function readCopyEvents(vaultRoot, lastDays) {
  const file = path__namespace.join(vaultRoot, LOG_FILE);
  let data = "";
  try {
    data = await fs$4.promises.readFile(file, "utf8");
  } catch {
    return [];
  }
  const now = Date.now();
  const min2 = now - lastDays * 24 * 60 * 60 * 1e3;
  const rows = [];
  for (const line of data.split(/\r?\n/)) {
    ...
    try {
      ...
      if (!(ev == null ? void 0 : ev.ts)) continue;
      const ts = Date.parse(ev.ts);
      if (!Number.isFinite(ts)) continue;
      if (ts >= min2) rows.push(ev);
    } catch {
    }
  }
...
electron.ipcMain.handle("copylog:list", async (_evt, args) => {
  const { modPlayVault, lastDays } = args || {};
  if (!modPlayVault) return [];
  return readCopyEvents(modPlayVault, Math.max(1, lastDays ?? 14));
});
async function appendCopyEvent(modPlayVault, ev) {
  const logFile = path__namespace$1.join(modPlayVault, ".copy_log.jsonl");
  const line = JSON.stringify(ev) + "\n";
  await require$$0$1.promises.mkdir(modPlayVault, { recursive: true });
  await require$$0$1.promises.appendFile(logFile, line, "utf8");
}
async function ensureDir$1(p) {
  await require$$0$1.promises.mkdir(p, { recursive: true });
}
async function* walk(dir) {
  const entries = await require$$0$1.promises.readdir(dir, { withFileTypes: true });
  for (const e of entries) {
    const full = path__namespace$1.join(dir, e.name);
    if (e.isDirectory()) yield* walk(full);
    else if (e.isFile()) yield full;
  }
}
async function mirrorPlatformConfigToModVault(srcRoot, destRoot) {
  ...
  let error;
  try {
    await ensureDir$1(destRoot);
    for await (const srcFile of walk(srcRoot)) {
      const rel = path__namespace$1.relative(srcRoot, srcFile);
      const destFile = path__namespace$1.join(destRoot, rel);
      await ensureDir$1(path__namespace$1.dirname(destFile));
      const data = await require$$0$1.promises.readFile(srcFile);
      bytes += data.byteLength;
      files += 1;
      await require$$0$1.promises.writeFile(destFile, data);
    }
  } catch (e) {
    result = "error";
    error = (e == null ? void 0 : e.message) || String(e);
  } finally {
    const ms = Math.round(perf_hooks.performance.now() - t0);
...
function resolveActiveDir(cfg) {
  if (cfg.activeModsPath) return cfg.activeModsPath;
  const legacy = cfg == null ? void 0 : cfg.paths;
  return (legacy == null ? void 0 : legacy.active_mods_path) ?? (legacy == null ? void 0 : legacy.activeModsPath) ?? (legacy == null ? void 0 : legacy.mods);
}
function resolveVaultDir(cfg) {
  if (cfg.modsVaultPath) return cfg.modsVaultPath;
  const legacy = cfg == null ? void 0 : cfg.vaults;
  return legacy == null ? void 0 : legacy.mods_vault;
}
async function readDirSafe(dir) {
  if (!dir) return [];
  try {
    return await fs$4.promises.readdir(dir, { withFileTypes: true });
  } catch {
    return [];
  }
}
async function listModFolders(root) {
  const entries = await readDirSafe(root);
  if (!root) return [];
  return entries.filter((e) => e.isDirectory()).map((e) => ({ id: e.name, dir: path$3.join(root, e.name) }));
}
safeHandle("mods:scanBoth", async () => {
  ...
  const rows = [
    ...active.map((r) => ({ ...r, source: "active" })),
    ...vault.map((r) => ({ ...r, source: "vault" }))
  ];
  return rows;
});
...
safeHandle("mods:watchStart", async () => {
  ...
  const vaultRoot = resolveVaultDir(cfg);
  await Promise.all([
    activeWatcher == null ? void 0 : activeWatcher.close().catch(() => {
    }),
    vaultWatcher == null ? void 0 : vaultWatcher.close().catch(() => {
    })
  ]);
  activeWatcher = null;
  vaultWatcher = null;
  if (activeRoot) {
    activeWatcher = chokidar.watch(activeRoot, { ignoreInitial: true, depth: 2 }).on("add", (p) => emitToAllWindows("mods:fs", { scope: "active", type: "add", p })).on("addDir", (p) => emitToAllWindows("mods:fs", { scope: "active", type: "addDir", p })).on("unlink", (p) => emitToAllWindows("mods:fs", { scope: "active", type: "unlink", p })).on("unlinkDir", (p) => emitToAllWindows("mods:fs", { scope: "active", type: "unlinkDir", p })).on("change", (p) => emitToAllWindows("mods:fs", { scope: "active", type: "change", p })).on("error", (e) => emitToAllWindows("mods:fs", { scope: "active", type: "error", e: String(e) }));
  }
  if (vaultRoot) {
    vaultWatcher = chokidar.watch(vaultRoot, { ignoreInitial: true, depth: 2 }).on("add", (p) => emitToAllWindows("mods:fs", { scope: "vault", type: "add", p })).on("addDir", (p) => emitToAllWindows("mods:fs", { scope: "vault", type: "addDir", p })).on("unlink", (p) => emitToAllWindows("mods:fs", { scope: "vault", type: "unlink", p })).on("unlinkDir", (p) => emitToAllWindows("mods:fs", { scope: "vault", type: "unlinkDir", p })).on("change", (p) => emitToAllWindows("mods:fs", { scope: "vault", type: "change", p })).on("error", (e) => emitToAllWindows("mods:fs", { scope: "vault", type: "error", e: String(e) }));
  }
...
electron.ipcMain.on("mods:emit", (_evt, payload) => emitToAllWindows("mods:fs", payload));
function resolveSaveRoot(cfg) {
  if (cfg.saveDataPath) return cfg.saveDataPath;
  const legacyPaths = cfg == null ? void 0 : cfg.paths;
  return (legacyPaths == null ? void 0 : legacyPaths.platform_config_root) ?? (legacyPaths == null ? void 0 : legacyPaths.config_root);
}
function resolveModPlayVault(cfg) {
  if (cfg.modPlayVaultPath) return cfg.modPlayVaultPath;
  const legacyVaults = cfg == null ? void 0 : cfg.vaults;
  return legacyVaults == null ? void 0 : legacyVaults.mod_play_vault;
}
...
const CONFIG_PATH = path$3.join(APP_DIR, "config.json");
function ensureDir(p) {
  if (p && !fs$4.existsSync(p)) fs$4.mkdirSync(p, { recursive: true });
}
function dirExists(p) {
  try {
    return fs$4.existsSync(p) && fs$4.statSync(p).isDirectory();
  } catch {
    return false;
  }
}
function readJSON(p, fallback) {
  try {
    return JSON.parse(fs$4.readFileSync(p, "utf-8"));
  } catch {
    return fallback;
  }
}
function writeJSON(p, data) {
  fs$4.writeFileSync(p, JSON.stringify(data, null, 2));
}
function listDirs(p) {
  if (!dirExists(p)) return [];
  return fs$4.readdirSync(p, { withFileTypes: true }).filter((d) => d.isDirectory()).map((d) => d.name);
}
function walkAllFiles(root) {
  ...
  while (st.length) {
    const cur = st.pop();
    for (const ent of fs$4.readdirSync(cur, { withFileTypes: true })) {
      const p = path$3.join(cur, ent.name);
...
function copyDir(src, dst) {
  ...
  ensureDir(dst);
  for (const e of fs$4.readdirSync(src, { withFileTypes: true })) {
    const s = path$3.join(src, e.name);
    const d = path$3.join(dst, e.name);
    if (e.isDirectory()) copyDir(s, d);
    else {
      ensureDir(path$3.dirname(d));
      fs$4.copyFileSync(s, d);
    }
  }
}
function emptyDir(dir) {
  if (!fs$4.existsSync(dir)) return;
  for (const e of fs$4.readdirSync(dir)) fs$4.rmSync(path$3.join(dir, e), { recursive: true, force: true });
}
function installModFiles(srcRoot, dstRoot, strategy) {
  const files = walkAllFiles(srcRoot);
  for (const f of files) {
    const rel = path$3.relative(srcRoot, f);
    const dst = path$3.join(dstRoot, rel);
    ensureDir(path$3.dirname(dst));
    try {
      if (strategy === "symlink") fs$4.symlinkSync(f, dst);
      else if (strategy === "hardlink") fs$4.linkSync(f, dst);
      else fs$4.copyFileSync(f, dst);
    } catch {
      fs$4.copyFileSync(f, dst);
    }
  }
}
function timestamp() {
...
function findExeInDir(dir) {
  for (const name of EXE_CANDIDATES) {
    const p = path$3.join(dir, name);
    if (fs$4.existsSync(p)) return p;
  }
  try {
    const files = fs$4.readdirSync(dir);
    const hit = files.find(
      (f) => f.toLowerCase().endsWith(".exe") && f.toLowerCase().includes("space") && f.toLowerCase().includes("marine") && f.includes("2")
    );
    return hit ? path$3.join(dir, hit) : null;
  } catch {
    return null;
  }
}
...
function parseLibraryFoldersVdf(vdfPath) {
  try {
    const txt = fs$4.readFileSync(vdfPath, "utf-8");
    return Array.from(txt.matchAll(/"path"\s+"([^"]+)"/g)).map((m) => path$3.join(m[1], "steamapps"));
  } catch {
    return [];
  }
}
function detectSm2ModsDir() {
  ...
  const candidates = [];
  if (steamPath) {
    const apps = path$3.join(steamPath, "steamapps");
    candidates.push(apps);
    const vdf = path$3.join(apps, "libraryfolders.vdf");
    if (fs$4.existsSync(vdf)) candidates.push(...parseLibraryFoldersVdf(vdf));
  }
  for (const apps of candidates) {
    const mods = path$3.join(apps, "common", "Space Marine 2", "client_pc", "root", "mods");
    if (fs$4.existsSync(mods)) return mods;
  }
  return null;
}
function detectSm2GameRootFromSteamLibraries() {
  ...
  const defaultApps = path$3.join(steamPath, "steamapps");
  appsCandidates.push(defaultApps);
  const vdf = path$3.join(defaultApps, "libraryfolders.vdf");
  if (fs$4.existsSync(vdf)) appsCandidates.push(...parseLibraryFoldersVdf(vdf));
  for (const apps of appsCandidates) {
    const root = path$3.join(apps, "common", "Space Marine 2");
...
function detectDefaultSavePath() {
  const base = path$3.join(os.homedir(), "AppData", "Local", "Saber", "Space Marine 2", "storage", "steam", "user");
  if (!fs$4.existsSync(base)) return "";
  const userDirs = fs$4.readdirSync(base, { withFileTypes: true }).filter((d) => d.isDirectory()).map((d) => d.name);
  if (!userDirs[0]) return "";
  const p = path$3.join(base, userDirs[0], "Main", "config");
  return fs$4.existsSync(p) ? p : "";
}
function autoDetectPaths() {
  ...
  if (modsDir) {
    ...
    const gameRoot = path$3.resolve(modsDir, "..", "..", "..");
    ...
    if (gameRoot) {
      detected.gameRoot = gameRoot;
      detected.gameExe = findExeInDir(gameRoot) ?? "";
      detected.activeModsPath = path$3.join(gameRoot, "client_pc", "root", "mods");
    }
  }
...
function backupSaveToPlayVault(modPlayVaultPath, saveDataPath) {
  ...
  const dst = path$3.join(modPlayVaultPath, timestamp());
...
function restoreLatestFromPlayVault(modPlayVaultPath, saveDataPath) {
  ...
  const subs = fs$4.readdirSync(modPlayVaultPath, { withFileTypes: true }).filter((d) => d.isDirectory()).map((d) => ({ name: d.name, mtime: fs$4.statSync(path$3.join(modPlayVaultPath, d.name)).mtimeMs })).sort((a, b) => b.mtime - a.mtime);
  if (!subs[0]) return;
  emptyDir(saveDataPath);
  copyDir(path$3.join(modPlayVaultPath, subs[0].name), saveDataPath);
}
let win = null;
function resolvePreloadPath() {
  for (const f of ["preload.js", "preload.cjs"]) {
    const p = path$3.join(__dirname, f);
    if (fs$4.existsSync(p)) return p;
  }
  return path$3.join(process.cwd(), "electron", "preload.ts");
}
async function createWindow() {
  ensureDir(APP_DIR);
  const cur = readJSON(CONFIG_PATH, defaultConfig);
  replaceConfig(cur);
  if (!cur.modsVaultPath) cur.modsVaultPath = path$3.join(APP_DIR, "mod_vault");
  if (!cur.modPlayVaultPath) cur.modPlayVaultPath = path$3.join(APP_DIR, "mod_play_vault");
...
safeHandle("fs:ensureDirs", (_e, paths) => {
  for (const p of paths) ensureDir(p);
  return true;
});
safeHandle("fs:testWrite", (_e, dir) => {
  try {
    ensureDir(dir);
    const probe = path$3.join(dir, `.write_test_${Math.random().toString(36).slice(2)}`);
    fs$4.writeFileSync(probe, "ok");
    fs$4.rmSync(probe, { force: true });
    return true;
  } catch {
    return false;
  }
});
safeHandle("browse:folder", async () => {
  const res = await electron.dialog.showOpenDialog({ properties: ["openDirectory"] });
  if (res.canceled || !res.filePaths.length) return null;
  return res.filePaths[0];
});
safeHandle("config:get", () => readJSON(CONFIG_PATH, defaultConfig));
safeHandle("config:set", (_e, cfg) => {
  writeJSON(CONFIG_PATH, cfg);
  return true;
});
safeHandle("config:completeSetup", (_e, cfg) => {
  const cur = readJSON(CONFIG_PATH, defaultConfig);
  const next = {
    ...cur,
    ...cfg,
...
electron.ipcMain.handle("mods:scan", () => {
  ...
  const list = [...names].map((name) => ({
    name,
    inMods: dirExists(path$3.join(cfg.activeModsPath, name)),
    inVault: dirExists(path$3.join(cfg.modsVaultPath, name))
  }));
...
electron.ipcMain.handle("mods:enable", (_e, modName) => {
  const cfg = readJSON(CONFIG_PATH, defaultConfig);
  const srcVault = path$3.join(cfg.modsVaultPath, modName);
  const dstMods = path$3.join(cfg.activeModsPath, modName);
...
electron.ipcMain.handle("mods:disable", (_e, modName) => {
  const cfg = readJSON(CONFIG_PATH, defaultConfig);
  const srcMods = path$3.join(cfg.activeModsPath, modName);
  const dstVault = path$3.join(cfg.modsVaultPath, modName);
  if (!dirExists(srcMods)) return true;
  if (!dirExists(dstVault)) copyDir(srcMods, dstVault);
  fs$4.rmSync(srcMods, { recursive: true, force: true });
  return true;
});
electron.ipcMain.handle("mods:delete", (_e, modName) => {
  const cfg = readJSON(CONFIG_PATH, defaultConfig);
  const inMods = path$3.join(cfg.activeModsPath, modName);
  const inVault = path$3.join(cfg.modsVaultPath, modName);
  if (dirExists(inMods)) fs$4.rmSync(inMods, { recursive: true, force: true });
  if (dirExists(inVault)) fs$4.rmSync(inVault, { recursive: true, force: true });
  return true;
});
electron.ipcMain.handle("mods:apply", (_e, enabledMods) => {
  ...
  for (const name of enabledMods) {
    const fromVault = path$3.join(cfg.modsVaultPath, name);
    const toMods = path$3.join(cfg.activeModsPath, name);
...
electron.ipcMain.handle("mods:launchTracked", async (_e, enabledMods) => {
  ...
  await new Promise((resolve) => {
    ...
    const child = node_child_process.spawn(cfg.gameExe, [], {
      cwd: path$3.dirname(cfg.gameExe),
      detached: false,
      stdio: "ignore"
    });
    child.on("close", () => {
